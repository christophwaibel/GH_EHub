...
'-  non-constant efficiencies, lifetime, costs... 
'-  take different technologies, still represented as one node (group)
'-      -> sequential opt., seperate run to identify specific entities of storage-group, HP-group, etc...

Public Class CaseB_2
    ...
    Public Sub RunCaseA(_YourInputPath As String, _period As Integer, _co2reduc As Double, _
                        ...
                        _TotFloorArea As Double, _allowCO2trading As Boolean)
        ...
        '       //Battery Data
        Dim CostBat As Double = 600                                 'Battery capital cost per kWh
        ...
        For p = 0 To Period - 1
            ...
            elecgenExpr.AddTerm(1, Discharge_Bat(p))
            elecdemandExpr.AddTerm(1, Charge_Bat(p))
            ...
            heatgenExpr.AddTerm(1, Discharge_TES(p))
            heatdemandExpr.AddTerm(1, Charge_TES(p))
        ...
        For p = 0 To Period - 2
            ...
            batstateExpr.AddTerm((1 - decay), Stored_Bat(p))
            batstateExpr.AddTerm(ch_eff, Charge_Bat(p))
            batstateExpr.AddTerm(-1 / disch_eff, Discharge_Bat(p))
            cpl.AddEq(Stored_Bat(p + 1), batstateExpr)
        Next
        cpl.AddGe(Stored_Bat(0), cpl.Prod(BatCap, min_state))           'initial state of battery >= min_state
        cpl.AddGe(Stored_Bat(0), Stored_Bat(Period - 1))                'initial state also >= state(end of year)
        ...
        For p = 0 To Period - 2
            ...
            tesstateExpr.AddTerm((1 - heatloss), Stored_TES(p))
            tesstateExpr.AddTerm(ceff_TES, Charge_TES(p))
            tesstateExpr.AddTerm(-1 / deff_TES, Discharge_TES(p))
            cpl.AddEq(Stored_TES(p + 1), tesstateExpr)
        Next
        ...
        If _allowCO2trading = True Then
            For p = 0 To Period - 1
                CO2Emissions.AddTerm(GridCO2EF, ElecPur(p))
                CO2Emissions.AddTerm(-GridCO2EF, ElecSell(p)) 'thats correct: if you sell into the grid your own electricity, its either coming from CHP, or PV or its been bought before from grid. CHP case, you already account for CO2 via generation using Fuel. PV is no CO2, and if its from grid, then your just giving back the CO2!!
