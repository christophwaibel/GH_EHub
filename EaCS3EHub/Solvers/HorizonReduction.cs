    ...
    public static class HorizonReduction
    {
        ...
        public static TypicalDays GenerateTypicalDays(double[][] fullProfiles, string[] loadTypes, int numberOfTypicalDays, bool[] peakDays, bool[] useForClustering, bool [] correctingLoad, bool verbose = true, string dataScaling = "standardization")
        {
            ...
            if (verbose)
            {
                ...
                Rhino.RhinoApp.WriteLine("{0} available threads for clustering...", Environment.ProcessorCount);
                Rhino.RhinoApp.WriteLine("");
            }
            Parallel.For(0, seeds.Length, seed =>
            {
                if (verbose) Rhino.RhinoApp.WriteLine("Performing clustering for seed {0} of {1} total seeds...", seed + 1, seeds.Length);
                replicates[seed] = EhubMisc.Clustering.KMedoids(Xclustering, clusters, 50, seed);
                if (verbose) Rhino.RhinoApp.WriteLine("...clustering finished for seed {0} of {1} total seeds.", seed + 1, seeds.Length);
            });
            ...
            {
                Rhino.RhinoApp.WriteLine("");
                Rhino.RhinoApp.WriteLine("Choosing cluster solution {0} for the MILP", minIndex);
                Rhino.RhinoApp.WriteLine("");
            }
